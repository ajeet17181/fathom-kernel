...
static struct bd6084gu_ctrl_tbl N_7[] = {
	...
	{0x0B, 0x08},	// (initial) CRV=log curve, Step=1.1mA
...
static void bd6084gu_go_opmode(struct bd6084gu_driver_data *drvdata)
{
	...
	switch (drvdata->mode) {
		case NORMAL_MODE:
			...
			break;
		case ALC_MODE:
			bd6084gu_set_table(drvdata, &N_7[0]);
			drvdata->state = NORMAL_STATE;
			break;
		default:
...
#define MAX_BRIGHTNESS          0x63

static struct backlight_properties eve_bl_props = {
       .max_brightness = MAX_BRIGHTNESS,
};

static int __devinit bd6084gu_probe(struct i2c_client *i2c_dev, const struct i2c_device_id *i2c_dev_id)
{
	...
	bd = backlight_device_register("bd6084gu-bl", &i2c_dev->dev, NULL, &bd6084gu_ops,&eve_bl_props);
