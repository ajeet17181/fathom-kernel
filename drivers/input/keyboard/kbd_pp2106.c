...
extern void ats_mtc_send_key_log_to_eta(struct ats_mtc_key_log_type* p_ats_mtc_key_log);
// ---------------------------------------------------------------------------
// LG_FW : khlee to support testmode
extern void set_opertion_mode(boolean isOnline);

#if defined (CONFIG_MACH_MSM7X27_ALOHAV) || defined (LG_FW_MTC)
...
/* LGE_CHANGE_S [jihoon.lee@lge.com] 2010-02-03, LG_FW_ATS_ETA_MTC_KEY_LOGGING */
#if defined (CONFIG_LGE_DIAGTEST) || defined (CONFIG_MACH_MSM7X27_ALOHAV) || defined (LG_FW_ATS_ETA_MTC_KEY_LOGGING)
...
static int  __devinit pp2106_probe(struct platform_device *pdev)
{
	int rc = 0;
	int key_idx;
